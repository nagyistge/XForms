...
namespace Xamarin.Forms.Platform.Mac
{
	public class VisualElementTracker : IDisposable
	{
		...
		private void SetElement (VisualElement oldElement, VisualElement newElement)
		{
			if (oldElement != null)
			{
				oldElement.PropertyChanged -= propertyChangedHandler;
				oldElement.SizeChanged -= sizeChangedEventHandler;
				oldElement.BatchCommitted -= batchCommittedHandler;
			}
			this.element = newElement;
			if (newElement == null)
				return;
			newElement.BatchCommitted += batchCommittedHandler;
			newElement.SizeChanged += sizeChangedEventHandler;
			newElement.PropertyChanged += propertyChangedHandler;
			this.UpdateNativeControl ();
		}
		...
		private void UpdateNativeControl ()
		{
			...
			if (this.NativeControlUpdated == null)
				return;
			// ISSUE: reference to a compiler-generated field
			this.NativeControlUpdated (this, EventArgs.Empty);
		}
