  ...
  public class PickerRenderer : ViewRenderer<Picker, UITextField>
  {
    ...
    private void UpdatePickerFromModel(PickerRenderer.PickerSource s)
    {
      if (this.Element == null)
        return;
      string text = this.Control.Text;
      this.Element.SetValueFromRenderer(Picker.SelectedIndexProperty, (object) s.SelectedIndex);
    ...
    private class PickerSource : UIPickerViewModel
    {
      ...
      {
        return (nint) (this.renderer.Element.get_Items() != null ? this.renderer.Element.get_Items().Count : 0);
      }
      ...
      {
        return this.renderer.Element.get_Items()[(int) row];
      }

      public override void Selected(UIPickerView picker, nint row, nint component)
      {
        if (this.renderer.Element.get_Items().Count == 0)
        {
        ...
        {
          this.SelectedItem = this.renderer.Element.get_Items()[(int) row];
